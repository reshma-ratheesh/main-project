...
if(isset($_POST['submit']))
{
	...
	$d=$_POST['city'];
	/*echo "$c$s$d";
	$query=mysqli_query($con,"SELECT `countryID` FROM `country` WHERE country_name='$c'");
	$cid=mysqli_insert_id($con);
	$query=mysqli_query($con,"SELECT `id` FROM `state` WHERE name='$s'");
	$sid=mysqli_insert_id($con);
	$query=mysqli_query($con,"SELECT `id` FROM `district` WHERE name='$d'");
